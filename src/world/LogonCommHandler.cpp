...
void LogonCommHandler::LoadRealmConfiguration()
{
    ...
    else
    {
		for(uint32 i = 1; i < realmcount+1; ++i)
		{
			...
			QueryResult *result = RealmDatabase.Query( "SELECT id, name, address, port, icon, color, timezone, population, realmbuilds FROM realmlist WHERE color <> 3 AND id = %u ORDER BY name", sMaster.realmID );
