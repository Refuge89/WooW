...
bool ChatHandler::HandleEventListEvents(const char* /*args*/, WorldSession* m_session)
{
    ...
    for (auto gameEventPair = eventList.begin(); gameEventPair != eventList.end(); ++gameEventPair)
    {
...
bool ChatHandler::HandleEventStartEvent(const char* args, WorldSession* m_session)
{
    ...
    for (auto eventPair = sGameEventMgr.mGameEvents.begin() ; eventPair != sGameEventMgr.mGameEvents.end() ; ++eventPair)
    {
