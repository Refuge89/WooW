...
{
    std::string query = "SELECT `entry`,`difficulty`,`minlevel`,`maxlevel`,`minhealth`,`maxhealth`,`mana`,`attacktime`,`mindamage`,`maxdamage`,";
    query.append("`rangedattacktime`,`rangedmindamage`,`rangedmaxdamage`,`armor`,`resistance1`,`resistance2`,`resistance3`,`resistance4`,");
    query.append("`resistance5`,`resistance6`,`resistance7`,`resistance8`,`money` FROM `creature_proto_difficulty`");

    Log.Notice("ObjectMgr", "Loading creature proto difficulty entries.");

    if (QueryResult* result = WorldDatabase.Query(query.c_str()))
    {
        do {
            Field* data = result->Fetch();
            uint32 entry = data[0].GetUInt32();
            if (!CreatureProtoStorage.LookupEntry(entry))
            {
                Log.Error("ObjectMgr", "Creature proto difficulty contains data for non existing creature %u. Skipping...", entry);
                continue;
            }

            // difficulty value check
            uint8 difficulty = data[1].GetUInt8();
            if (difficulty > TOTAL_RAID_MODES)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty entry %u has non supported difficulty %u. Skipping...", entry, difficulty);
                continue;
            }
            else if (difficulty == MODE_NORMAL_10MEN)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty entry %u has normal mode difficulty (%u). Skipping...", entry, difficulty);
                continue;
            }

            CreatureProtoDifficulty* difficultyData = NULL;
            difficultyData->minlevel = data[2].GetUInt8();
            if (!difficultyData->minlevel)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty (entry: %u, difficulty: %u) contains wrong minlevel.", entry, difficulty);
                continue;
            }

            difficultyData->maxlevel = data[3].GetUInt8();
            if (!difficultyData->maxlevel || difficultyData->minlevel > difficultyData->maxlevel)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty (entry: %u, difficulty: %u) contains wrong maxlevel.", entry, difficulty);
                continue;
            }

            difficultyData->minhealth = data[4].GetUInt32();
            if (!difficultyData->minhealth)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty (entry: %u, difficulty: %u) contains wrong minhealth.", entry, difficulty);
                continue;
            }

            difficultyData->maxhealth = data[5].GetUInt32();
            if (!difficultyData->maxhealth || difficultyData->minhealth > difficultyData->maxhealth)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty (entry: %u, difficulty: %u) contains wrong maxlevel.", entry, difficulty);
                continue;
            }

            difficultyData->mana = data[6].GetUInt32();
            difficultyData->attacktime = data[7].GetUInt32();
            if (!difficultyData->attacktime)
            {
                Log.Error("ObjectMgr", "Creature proto difficulty (entry: %u, difficulty: %u) contains wrong attack time.", entry, difficulty);
                continue;
            }

            difficultyData->mindamage = data[8].GetUInt32();
            difficultyData->maxdamage = data[9].GetUInt32();
            difficultyData->rangedattacktime = data[10].GetUInt32();
            difficultyData->rangedmindamage = data[11].GetUInt32();
            difficultyData->rangedmaxdamage = data[12].GetUInt32();
            for (uint8 i = 0; i < SCHOOL_COUNT; i++)
                difficultyData->resistance[i] = data[13 + i].GetUInt32();
            difficultyData->money = data[20].GetUInt32();
            CreatureDifficultyData[entry] = std::make_pair(difficulty, difficultyData);
        } while (result->NextRow());
        delete result;
    }
    Log.Success("ObjectMgr", "Loaded %u creature_proto_difficulty data", CreatureDifficultyData.size());
}

CreatureProtoDifficulty* ObjectMgr::GetCreatureDiffData(uint32 entry, uint8 difficulty)
{
    for (uint8 i = 0; i < 4; i++)
        if (CreatureDifficultyData[entry].first == difficulty)
            return CreatureDifficultyData[entry].second;

    Log.Debug("ObjectMgr", "Cannot find creature (entry: %u, difficulty %u) difficulty data. Creature will use standart stats.", entry, difficulty);
    return NULL;
}
