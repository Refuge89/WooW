...
bool Player::Create(WorldPacket & data)
{
	...
	{
		setAction(static_cast<uint8>(itr->button), static_cast<uint16>(itr->action), static_cast<uint8>(itr->misc), static_cast<uint8>(itr->type));
	}
...
void Player::_EventExploration()
{
	...
	switch(at->ZoneId)
	{
		...
		{
			if(at && at->AreaId == 4342 &&  at->explorationFlag == 2048 )
			{
				if(pPlayer->getClass() == DEATHKNIGHT  && !pPlayer->HasAura(51721) )
				{
					if(pPlayer->HasSpell(51721)  && !pPlayer->GetTaxiState() )
					{
						CastSpell(pPlayer, 51721, false);
					}
				}
			}else
				RemoveAura(51721);
...
//	sHookInterface.OnZone(this, m_zoneId, 0); // av on ascent
...
void Player::setAction(uint8 button, uint16 action, uint8 misc, uint8 type)
{
