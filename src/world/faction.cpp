...
bool isAttackable(Object* objA, Object* objB, bool CheckStealth)// A can attack B?
{
	...
	if(objA->IsUnit())
	{
		if( objA->HasFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_9 | UNIT_FLAG_MOUNTED_TAXI | UNIT_FLAG_NOT_SELECTABLE))
			return false;
	...
	if( CheckStealth && objB->IsUnit() && TO_UNIT(objB)->IsStealth() )
		return false;
...
Player* GetPlayerFromObject(Object* obj)
{
	...
	else if( obj->IsPet() )
	{
		...
		if( pet_obj )
			...
			player_obj =  TO_PLAYER(pet_obj->GetPlayerOwner());
	}
	else if( obj->IsUnit() )
	{
		...
		if( creature_obj && creature_obj->IsTotem()) 
			...
			player_obj =  TO_PLAYER(creature_obj->GetPlayerOwner());
	}
	return player_obj;
}
