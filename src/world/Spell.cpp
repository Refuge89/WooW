...
void Spell::SendChannelUpdate(uint32 time)
{
	...
	//		if(dynObj == NULL && u_caster->tmpAura.find(m_spellInfo->Id) == u_caster->tmpAura.end())  //no persistant aura or aura on caster
			{
...
{
	Unit * Target = NULL;
	if(guid == 0 || m_pendingAuras.find(guid) != m_pendingAuras.end())
	{
		DecRef();
		return;
	}

	m_pendingAuras.insert(guid);
	...
	if(itr!=Target->tmpAura.end())
	{
		if(itr->second != NULL)
		{
			if(itr->second->GetSpellProto()->procCharges>0)
			{
				...
				{
					Aura* staur = sSpellFactoryMgr.NewAura(itr->second->GetSpellProto(), itr->second->GetDuration(), itr->second->GetCaster(), itr->second->GetTarget(), m_triggeredSpell, i_caster);
...
uint8 Spell::CanCast(bool tolerate)
{
	...
	if(p_caster)
	{
		...
		{
			AreaTable* at = p_caster->GetMapMgr()->GetArea(p_caster->GetPositionX(), p_caster->GetPositionY(), p_caster->GetPositionZ());
...
/*
void Spell::HandlesCastEffects(uint32 i)
{   
	// Try a dummy SpellHandler
	if(sScriptMgr.CallScriptedDummySpell(m_spellInfo->Id, i, this ))
	{
