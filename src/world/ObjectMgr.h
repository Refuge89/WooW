...
struct CreatureProtoDifficulty
{
    ...
    uint32 rangedmindamage;
    uint32 rangedmaxdamage;
    uint32 resistance[SCHOOL_COUNT];
    uint32 money;
};

typedef std::map< uint32, std::pair<uint8, CreatureProtoDifficulty*> > CreatureDifficultyStruct;
...
class SERVER_DECL ObjectMgr : public Singleton < ObjectMgr >, public EventableObject
{
	public:
		...
		// Creature difficulty stats system
		...
		CreatureProtoDifficulty* GetCreatureDiffData(uint32 entry, uint8 difficulty);
...
#define ENABLE_ALWAYS_SERIOUS_MODE_GCC_STL_HACK
	...
	protected:
		...
		CreatureDifficultyStruct CreatureDifficultyData;
