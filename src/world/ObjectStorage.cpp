...
// cebernic: plz feedback
{
	...
	{
		Log.Basic("ObjectMgr", "BCSystem Disabled.");
		return;
	}
	// ----------------
	...
	if(keyGroup.empty())
	{
		Log.Basic("ObjectMgr", "BCSystem error! worldbroadcast empty? fill it first!");
		sWorld.BCSystemEnable = false;
		return;
	}
	else
		Log.Basic("ObjectMgr", "BCSystem Enabled with %u KeyGroups.", keyGroup.size());
