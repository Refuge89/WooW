...
bool Master::Run(int argc, char** argv)
{
	...
	if(do_check_conf)
	{
		Log.Basic("Config", "Checking config file: %s", config_file);
		if(Config.MainConfig.SetSource(config_file, true))
			Log.Success("Config", "Passed without errors.");
		else
			Log.Error("Config", "Encountered one or more errors.");

		Log.Basic("Config", "Checking config file: %s", realm_config_file);
		if(Config.RealmConfig.SetSource(realm_config_file, true))
			Log.Success("Config", "Passed without errors.");
		else
			Log.Error("Config", "Encountered one or more errors.");

		Log.Basic("Config", "Checking config file:: %s", optional_config_file);
	...
	Log.Basic("Rnd", "Initialized Random Number Generators.");
	...
	if(Config.MainConfig.SetSource(config_file))
		Log.Basic("Config", ">> " CONFDIR "/world.conf loaded");
	else
	{
	...
	if(Config.OptionalConfig.SetSource(optional_config_file))
		Log.Basic("Config", ">> " CONFDIR "/optional.conf loaded");
	else
	{
	...
	if(Config.RealmConfig.SetSource(realm_config_file))
		Log.Basic("Config", ">> " CONFDIR "/realms.conf loaded");
	else
	{
...
//	Log.Success("Server", "Ready for connections. Startup time: %ums", LoadingTime);
	...
	Log.Basic("RemoteConsole", "Starting...");
	if(StartConsoleListener())
	{
#ifdef WIN32
		ThreadPool.ExecuteTask(GetConsoleListener());
#endif
		Log.Basic("RemoteConsole", "Now open.");
	}
	else
	{
			...
			if(getMSTime() >= next_printout)
			{
				if(m_ShutdownTimer > 60000.0f)
				{
					if(!((int)(m_ShutdownTimer) % 60000))
						Log.Basic("Server", "Shutdown in %i minutes.", (int)(m_ShutdownTimer / 60000.0f));
				}
				else
					Log.Basic("Server", "Shutdown in %i seconds.", (int)(m_ShutdownTimer / 1000.0f));
	...
	{
		Log.Basic("Shutdown", "Waiting for loot to finish loading...");
		while(lootmgr.is_loading)
			Arcemu::Sleep(100);
	}

	// send a query to wake it up if its inactive
	Log.Basic("Database", "Clearing all pending queries...");

	// kill the database thread first so we don't lose any queries/data
	CharacterDatabase.EndThreads();
	WorldDatabase.EndThreads();

	Log.Basic("DayWatcherThread", "Exiting...");
	dw->terminate();
	dw = NULL;

	Log.Basic("CommonScheduleThread", "Exiting...");
	cs->terminate();
	cs = NULL;
	...
	Log.Basic("Network", "Shutting down network subsystem.");
#ifdef WIN32
	...
	Log.Basic("World", "~World()");
	delete World::getSingletonPtr();

	sScriptMgr.UnloadScripts();
	delete ScriptMgr::getSingletonPtr();

	Log.Basic("ChatHandler", "~ChatHandler()");
	delete ChatHandler::getSingletonPtr();

	Log.Basic("EventMgr", "~EventMgr()");
	delete EventMgr::getSingletonPtr();

	Log.Basic("Database", "Closing Connections...");
	_StopDB();

	Log.Basic("Network", "Deleting Network Subsystem...");
...
void OnCrash(bool Terminate)
{
	...
	try
	{
		...
		{
			Log.Basic("sql", "Waiting for all database queries to finish...");
			WorldDatabase.EndThreads();
			CharacterDatabase.EndThreads();
			Log.Basic("sql", "All pending database operations cleared.");
			sWorld.SaveAllPlayers();
			Log.Basic("sql", "Data saved.");
		}
	}
	catch(...)
	{
