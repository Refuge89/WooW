...
void WorldSession::HandleMovementOpcodes(WorldPacket & recv_data)
{
	...
	if(!_player->IsInWorld() || _player->GetCharmedByGUID() || 
		(_player->GetPlayerStatus() == TRANSFER_PENDING && _player->m_CurrentVehicle == NULL) || _player->GetTaxiState() || _player->getDeathState() == JUST_DIED)
		return;
		...
		if(recv_data.GetOpcode() == MSG_MOVE_FALL_LAND)
		{
				...
				( ( mover->GetGUID() != _player->GetGUID() ) || ( !_player->GodModeCheat && ( UNIXTIME >= _player->m_fallDisabledUntil ) ) ) )
			{
				...
				if( _player->m_CurrentVehicle )
					mover = _player->m_CurrentVehicle;
	...
	if( ( mover->transporter_info.guid != 0 ) && ( movement_info.transGuid.GetOldGuid() == 0 ) ){
		...
		if( movement_info.transGuid.GetOldGuid() != 0 ){
			...
			else if( mover->transporter_info.guid == 0 ){
