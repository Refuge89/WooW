...
bool ChatHandler::HandleDebugSpawnWarCommand(const char* args, WorldSession* m_session)
{
	...
	for(; count > 0; --count)
	{
		...
		c->Load(cp, m_session->GetPlayer()->GetMapMgr(), bx + x, by + y, z, 0.0f);
		if(health != 0)
		{
...
bool ChatHandler::HandleSneekOn(const char*args, WorldSession * m_session)
{
	Player* p = m_session->GetPlayer();

	p->SetFlag(UNIT_FIELD_BYTES_2,0x1E000000);//sneak anim
	p->SetFlag(UNIT_FIELD_BYTES_1, 0x020000);
	p->SetFlag(PLAYER_FIELD_BYTES2, 0x2000);
