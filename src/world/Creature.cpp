...
void Creature::generateLoot()
{
    ...
    switch (GetMapMgr()->iInstanceMode)
    {
		case MODE_NORMAL: loot.gold = proto->money; break;// MODE_NORMAL_10MEN
		case MODE_HEROIC:    // MODE_NORMAL_25MEN
		{
			if (GetMapMgr()->GetMapInfo()->HasDifficulty(MODE_NORMAL_10MEN)) // indicates normal mode, which means its heroic or 25 man normal mode
			{
				if (CreatureProtoDifficulty* stats = objmgr.GetCreatureDiffData(GetEntry(), GetMapMgr()->iInstanceMode))
					loot.gold = stats->money;
				else
					loot.gold = proto->money;
			}
			else if (!GetMapMgr()->GetMapInfo()->HasDifficulty(MODE_HEROIC_10MEN))// we have 25 man mode here, not setting heroic data
				loot.gold = proto->money;
		}break;
		// only heroic data here
		case MODE_HEROIC_10MEN:
		case MODE_HEROIC_25MEN:
		{
			if (CreatureProtoDifficulty* stats = objmgr.GetCreatureDiffData(GetEntry(), GetMapMgr()->iInstanceMode))
...
bool Creature::Load(CreatureSpawn* spawn, uint32 mode, MapInfo* info)
{
    ...
    switch (mode)
    {
        case MODE_NORMAL: // MODE_NORMAL_10MEN
        {
            uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
            setLevel(level_to_set);

            uint32 health = proto->MinHealth + (proto->MaxHealth - proto->MinHealth) / ((proto->MaxLevel - level_to_set)+1);
        ...
        case MODE_HEROIC:    // MODE_NORMAL_25MEN
        {
            if (info->HasDifficulty(MODE_NORMAL_10MEN)) // indicates normal mode, which means its heroic or 25 man normal mode
            {
                if (CreatureProtoDifficulty* stats = objmgr.GetCreatureDiffData(GetEntry(), mode))
                {
                    uint8 level_to_set = stats->minlevel + RandomUInt(stats->maxlevel - stats->minlevel);
                    setLevel(level_to_set);

                    uint32 health = stats->minhealth + (stats->maxhealth - stats->minhealth) / ((stats->maxlevel - level_to_set) + 1);
                    SetHealth(health);
                    SetMaxHealth(health);
                    SetBaseHealth(health);
                    SetMaxPower(POWER_TYPE_MANA, stats->mana);
                    SetBaseMana(stats->mana);
                    SetPower(POWER_TYPE_MANA, stats->mana);

                    for (uint8 i = 0; i < SCHOOL_COUNT; ++i)
                        SetResistance(i, stats->resistance[i]);

                    SetBaseAttackTime(MELEE, stats->attacktime);
                    SetMinDamage(stats->mindamage);
                    SetMaxDamage(stats->maxdamage);
                    SetBaseAttackTime(RANGED, stats->rangedattacktime);
                    SetMinRangedDamage(stats->rangedmindamage);
                    SetMaxRangedDamage(stats->rangedmaxdamage);
                }
                else
                {
                    uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
            ...
            else if (!info->HasDifficulty(MODE_HEROIC_10MEN))// if we don't have 10 man heroic - applying normal proto data
            {
                uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
                setLevel(level_to_set);
        ...
        default:
        {
                Log.Debug("Creature", "Cannot get creature data for undescribed instance difficulty %u. Creature (entry: %u) will use default proto data", mode, proto->Id);
                uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
                setLevel(level_to_set);
    ...
    GetAIInterface()->m_canRangedAttack = proto->CanRanged ? true : false;
...
void Creature::Load(CreatureProto* proto_, MapMgr* mgr, float x, float y, float z, float o)
{
    proto = proto_;
    ...
    switch (mgr->iInstanceMode)
    {
        ...
        case MODE_HEROIC:    // MODE_NORMAL_25MEN
        {
            if (mgr->GetMapInfo()->HasDifficulty(MODE_NORMAL_10MEN)) // indicates normal mode, which means its heroic or 25 man normal mode
            {
                if (CreatureProtoDifficulty* stats = objmgr.GetCreatureDiffData(GetEntry(), mgr->iInstanceMode))
                {
                    uint8 level_to_set = stats->minlevel + RandomUInt(stats->maxlevel - stats->minlevel);
                    setLevel(level_to_set);
            ...
            else if (!mgr->GetMapInfo()->HasDifficulty(MODE_HEROIC_10MEN))// if we don't have 10 man heroic - applying normal proto data
            {
                uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
                setLevel(level_to_set);
        ...
        default:
        {
                   Log.Debug("Creature", "Cannot get creature data for undescribed instance difficulty %u. Creature (entry: %u) will use default proto data", mgr->iInstanceMode, proto->Id);
            uint8 level_to_set = proto->MinLevel + RandomUInt(proto->MaxLevel - proto->MinLevel);
            setLevel(level_to_set);
