    ...
    public:
        ...
        void LoadAdditionalData(const char* IndexName, const char* FormatString)
        {
            ...
            if(result->GetFieldCount() != cols)
            {
                if(result->GetFieldCount() > cols)
                {
                    Log.Debug("Storage", "Invalid format in %s (%u/%u), loading anyway because we have enough data", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
                }
                else
                {
                    Log.Error("Storage", "Invalid format in %s (%u/%u), not enough data to proceed.", IndexName, (unsigned int)cols, (unsigned int)result->GetFieldCount());
        ...
        void Reload()
        {
            Log.Debug("Storage", "Reloading database cache from `%s`...", Storage<T, StorageType>::_indexName);
