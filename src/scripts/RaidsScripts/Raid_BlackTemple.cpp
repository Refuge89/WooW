...
void Matar4firstboss(uint32 count, Creature* unitx)
{
	   ...
	   if (opengatebt<=0)
	   {
			GameObject* pDoorBT = unitx->GetMapMgr()->GetInterface()->GetGameObjectNearestCoords(855.715f, 392.976f, 112.784f, 185479);
			if(pDoorBT == 0)
					return;
...
class VolcanoAI : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(VolcanoAI);

    VolcanoAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
...
class MoltenFlameAI : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(MoltenFlameAI);

    MoltenFlameAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		_unit->SetUInt64Value(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_ATTACKABLE_2);
...
class ReliquaryoftheLostAI : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(ReliquaryoftheLostAI);

    ReliquaryoftheLostAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
...
public:
	...
	void OnCombatStop(Unit* mTarget)
    {
		Creature* boss = _unit->GetMapMgr()->GetInterface()->GetCreatureNearestCoords(496.4747f, 181.1151f, 94.05f, CN_RELIQUARYOFTHELOST);
		if( boss )
...
//			}   
/*
			Aura* aura = new Aura(dbcSpell.LookupEntry(FIXATE), 5000, _unit, RTarget);
			aura->m_areaAura = false;
			RTarget->AddAura(aura);
			aura->AddMod(4, 1, 0, 0);
			aura->AddMod(4, 0, 0, 1);*/ 
...
/*
			Aura* aura = new Aura(dbcSpell.LookupEntry(SPITE_IMM), true);
			aura->m_areaAura = false;
...
public:
    ...
    void OnDied(Unit* mKiller)
    {
		...
		Spell* spell = new Spell(_unit,dbcSpell.LookupEntry(41542),true,NULL);
		//Spell* spell=new Spell(_unit,dbcSpell.LookupEntry(41542),true,NULL);
    ...
    void AIUpdate()
    {
		...
		{ 
			if ( (*itr) && (*itr)->IsPlayer() )
			{
				...
				{
					float dist = tg->GetDistance2dSq( static_cast< Object* >( _unit ) );
					if( dist <= 150.0f )
...
class DOOMBLOSSOMAI : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(DOOMBLOSSOMAI);

    DOOMBLOSSOMAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		RegisterAIUpdateEvent(5000);			
    }
...
class ShadeofakamaAI3 : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(ShadeofakamaAI3);
	SP_AI_Spell spells[1];
	bool m_spellcheck[1];

    ShadeofakamaAI3(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		nrspells = 1;
		for(int i=0;i<nrspells;i++)
		{
...
class AshtongueSorcererAI : public CreatureAIScript
{
public:
    ...
    AshtongueSorcererAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		_unit->m_canMove = false;
		_unit->GetAIInterface()->disable_melee = true;
...
class TriggerShadeofAkamaAI : public CreatureAIScript
{
public:
    ADD_CREATURE_FACTORY_FUNCTION(TriggerShadeofAkamaAI);
    TriggerShadeofAkamaAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		_unit->GetAIInterface()->SetAllowedToEnterCombat(false);
		_unit->SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_NOT_SELECTABLE);
		_unit->GetAIInterface()->m_canMove = false;
...
class AkamaFakeAI : public CreatureAIScript
{
public:
    ...
    AkamaFakeAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		nrspells = 1;
		for(int i=0;i<nrspells;i++)
		{
...
class BrokenAshtongueAI : public CreatureAIScript
{
public:
	ADD_CREATURE_FACTORY_FUNCTION(BrokenAshtongueAI);
    BrokenAshtongueAI(Creature* pCreature) : CreatureAIScript(pCreature)
    {
		_unit->GetAIInterface()->SetAllowedToEnterCombat(false);
    ...
    void OnDied(Unit* mKiller)
    {
		...
		if (!DeathEvent[_unit->GetInstanceID()] && MaievDialog[_unit->GetInstanceID()] >= 28 && Maiev && Maiev->isAlive())	// it's workaround (added pointer check, coz if he dies when Maiev is not on battlefield then it will crash server)
		{
			...
			// azolex to prevent compile error
				uint32 nullfix = 0;
			_unit->GetAIInterface()->resetNextTarget(); //  do7me fix
			_unit->GetAIInterface()->m_canMove = false;
