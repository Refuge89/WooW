				...
				if (_unit->GetMapMgr()->CreatureStorage[i] != NULL)
				{
					...
					if( entry == 31126 || entry == 31127 || entry == 28167 || entry == 28169 )
					{
						...
						if (cp && ci)
						{
							...
							if (c) {
								//position is guessed
								c->Load(cp, _unit->GetMapMgr(), citizen->GetPositionX(), citizen->GetPositionY(), citizen->GetPositionZ(), citizen->GetOrientation());
	...
	if( crate )
	{
		...
		{//weee, Uther
			...
			if (cp && ci)
			{
				...
				if (c) {
					//position is guessed
					c->Load(cp,plr->GetMapMgr(), 1759.4351f, 1265.3317f, 138.052f, 0.1902f);
		...
		case 2:
			{
				...
				if (cp && ci)
				{
					...
					if (c) {
						//position is guessed
						c->Load(cp,_unit->GetMapMgr(), 2113.52f, 1288.01f, 136.382f, 2.30383f);
