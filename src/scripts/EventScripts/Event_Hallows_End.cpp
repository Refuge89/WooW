...
class ShadeOfTheHorsemanAI : public MoonScriptCreatureAI
{
    ...
    ShadeOfTheHorsemanAI(Creature* pCreature) : MoonScriptCreatureAI(pCreature)
    {
        ...
        AreaTable* area = dbcArea.LookupEntry(((Player*)_unit)->GetAreaID()); // _unit->GetArea();
        if (area != nullptr)
        {
            switch (area->AreaId)
            {
    ...
    //    auto area = _unit->GetArea();
        AreaTable*  area = dbcArea.LookupEntry(((Player*)_unit)->GetAreaID());
        uint32 area_id = area ? area->AreaId : 0;
    ...
    void OnDied(Unit* pKiller)
    {
        GameObject* Pumpkin = sEAS.SpawnGameobject(static_cast<Player*>(pKiller), 2883, _unit->GetPositionX() + RandomFloat(5.0f), _unit->GetPositionY() + RandomFloat(5.0f), _unit->GetPositionZ(), 0, 1, 0, 0, 0, 0);
        if (Pumpkin != nullptr)
        ...
        void OnActivate(Player* pPlayer)
        {
            ...
            else
            {
                if (pPlayer->GetItemInterface()->GetItemCount(32971, false) == 0)
                {
                    ...
                    AddItemResult result = pPlayer->GetItemInterface()->SafeAddItem(itm, slotresult.ContainerSlot, slotresult.Slot);
                    if (!result)
                    {
