...
void Warper2::GetTeleportEntries(uint32 creature, uint32 parent,std::vector<ct_entry> &entries)
{
        QueryResult * res;
        res = WorldDatabase.Query("SELECT * FROM `caladriel_teleporter` WHERE creature='%u' AND parent='%u' LIMIT %u",creature,parent,MAX_ENTRIES);
        if(res)
        {
                do
                {
...
void Warper2::GetTeleportEntry(uint32 creature, uint32 id, ct_entry &entry)
{
        QueryResult * res;
        res = WorldDatabase.Query("SELECT * FROM `caladriel_teleporter` WHERE creature='%u' AND menuid='%u'",creature,id);
	ProcessEntryData(creature,res,entry);
	delete res;
}

bool Warper2::CanDisplay(ct_entry entry, Player * Plr)
{
...
bool Warper2::ProcessEntryData(uint32 creature, QueryResult * res,ct_entry &entry)
{
	if(res)
	{
		QueryResult * res2;
		bool hasChildren = false;
		res2 = WorldDatabase.Query("SELECT menuid FROM `caladriel_teleporter` WHERE creature='%u' AND parent='%u'", creature,res->Fetch()[2].GetUInt32());
...
bool Warper2::IsValid(QueryResult * res)
{
...
void SetupWarper2(ScriptMgr * mgr)
{
	...
	QueryResult * res = WorldDatabase.Query("SELECT DISTINCT(`creature`) FROM `caladriel_teleporter`");
        if(res)
        {
                do
                {
