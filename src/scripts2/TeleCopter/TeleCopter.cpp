...
{
        QueryResult * res;
        res = WorldDatabase.Query("SELECT * FROM `telecopter` WHERE creature='%u' AND parent='%u' LIMIT %u",creature,parent,MAX_ENTRIES);
        if(res)
        {
                do
                {
...
{
        QueryResult * res;
        res = WorldDatabase.Query("SELECT * FROM `telecopter` WHERE creature='%u' AND menuid='%u'",creature,id);
...
bool Warper2::ProcessEntryData(uint32 creature, QueryResult * res,ct_entry &entry)
{
	if(res)
	{
		...
		res2 = WorldDatabase.Query("SELECT menuid FROM `telecopter` WHERE creature='%u' AND parent='%u'", creature,res->Fetch()[2].GetUInt32());
...
void SetupWarper2(ScriptMgr * mgr)
{
	...
	QueryResult * res = WorldDatabase.Query("SELECT DISTINCT(`creature`) FROM `telecopter`");
        if(res)
        {
                do
                {
